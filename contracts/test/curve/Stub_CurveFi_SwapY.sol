...
import "./Stub_CurveFi_LPTokenY.sol";
...
contract Stub_CurveFi_SwapY {
    ...
    constructor(address[N_COINS] memory _coins, address[N_COINS] memory _underlying_coins, address _pool_token, uint256 _fee) public {
    ...
    function add_liquidity(uint256[N_COINS] memory amounts, uint256 min_mint_amount) public {
        ...
        Stub_CurveFi_LPTokenY(__token).mint(msg.sender, mint_amount);
    }
    ...
    function remove_liquidity (uint256 _amount, uint256[N_COINS] memory min_amounts) public {
        ...
        for (uint256 i=0; i < __coins.length; i++){
            ...
            IYErc20(__coins[i]).transfer(msg.sender, amounts[i]);
        }
        ERC20Burnable(__token).burnFrom(msg.sender, _amount);
    }
    ...
    function remove_liquidity_imbalance(uint256[N_COINS] memory amounts, uint256 max_burn_amount) public {
        ...
        ERC20Burnable(__token).burnFrom(msg.sender, token_amount);
    }
...
//            diff = D0 - D2;
    ...
    function normalize(address coin, uint256 amount) internal view returns(uint256) {
        uint8 decimals = ERC20(coin).decimals();
        if (decimals == 18) {
            return amount;
        } else if (decimals > 18) {
