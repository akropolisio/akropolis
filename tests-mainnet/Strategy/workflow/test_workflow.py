...
def test_full_workflow(yearnVault, regular_user, lpToken, lptoken_owner, Governance, vault_savings, owner,
    ...
    with brownie.reverts("Depositing zero amount"):
        vault_savings.deposit["address,uint"](contract.address, 0, {"from": regular_user})

    contract.approve(vault_savings.address, contract.balanceOf(vault_savings.address), {"from": regular_user})
    with brownie.reverts("Withdrawing zero amount"):
        vault_savings.withdraw["address,uint"](contract.address, 0, {"from": regular_user})
