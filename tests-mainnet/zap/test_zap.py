...
def test_zap_high_liquidity(
    zap, user1, yearn_vault, sellTokenName, buyTokenName, whale, curve_address, vault_owner, amount_in, deployer, amount_out
):
    ...
    zap.setApprovedTokens([sellToken, buyToken], [True, True], {"from": deployer})
...
@parametrize_from_file(data, "data_zap_high")
def test_zap_out(zap, user1, yearn_vault, sellTokenName, buyTokenName, vault_owner, curve_address, whale, zapperData, amount_in, amount_out, deployer):
    vault = Contract.from_explorer(yearn_vault, as_proxy_for=None)
    curve_swap_address = Contract.from_explorer(curve_address, as_proxy_for=None)
...
@parametrize_from_file(data, "data_zap_high")
def test_unregister_zapIn_coin(zap, user1, yearn_vault, sellTokenName, buyTokenName, whale, curve_address, vault_owner, amount_in, deployer, amount_out):
    curve_swap_address = Contract.from_explorer(curve_address, as_proxy_for=None)
    vault = Contract.from_explorer(yearn_vault, as_proxy_for=None)
...
@parametrize_from_file(data, "data_zap_high")
def test_unregister_zapOut_coin(zap, user1, yearn_vault, sellTokenName, buyTokenName, vault_owner, curve_address, whale, zapperData, amount_in, amount_out, deployer):
    vault = Contract.from_explorer(yearn_vault, as_proxy_for=None)
    curve_swap_address = Contract.from_explorer(curve_address, as_proxy_for=None)
    account_owner = accounts.at(vault_owner, force=True)
...
def test_slippage_zapIn(zap, user1, yearn_vault, sellTokenName, buyTokenName, vault_owner, curve_address, whale, zapperData, amount_in, amount_out, deployer):
    ...
    tokenToSell.approve(zap, sellAmount, {"from": user1})
    min_amount = curve_swap_address.calc_token_amount([int(buyAmount), 0, 0], True)
    amount_to_test = min_amount * 3
    assert amount_to_test > min_amount


    #add the token to approve array
    with brownie.reverts("slippage error"):
    # start zapIn
        zap.zapIn(
            sellToken,
            buyToken,
