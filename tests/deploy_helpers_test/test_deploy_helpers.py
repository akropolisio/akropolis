...
def test_helpers_with_admin_contract(
    deployer, admin_contract, UpgradeTestOld, UpgradeTestNew
):
    init_arg = 10
    testImplFromProxy, testProxy, testImpl = deploy_proxy(
        deployer, admin_contract, UpgradeTestOld, init_arg
    )
    ...
    newTestImplFromProxy, newTestImpl = upgrade_proxy(
        deployer, admin_contract, testProxy, UpgradeTestNew
    )

    assert (
        testProxy.implementation.call({"from": admin_contract}) == newTestImpl.address
    )
...
def test_helpers_with_admin_address(
    deployer, admin_address, UpgradeTestOld, UpgradeTestNew
):
    init_arg = 20
    testImplFromProxy, testProxy, testImpl = deploy_proxy(
        deployer, admin_address, UpgradeTestOld, init_arg
    )
    ...
    newTestImplFromProxy, newTestImpl = upgrade_proxy(
        deployer, admin_address, testProxy, UpgradeTestNew
    )
