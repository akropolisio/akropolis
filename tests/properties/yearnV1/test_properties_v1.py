...
def test_deposit_unpause_vault_reverts(token, vault, vaultSavings, deployer, user):
    ...
    with brownie.reverts():
        vaultSavings.deposit["address,uint"](
            vault.address, DEPOSIT_VALUE, {"from": user}
        )
...
@given(user=strategy("address", length=10))
@settings(max_examples=50)
def test_deposit_array_unpause_vault_reverts(
    token, vault, vaultSavings, deployer, user
):
    vaultSavings.pause({"from": deployer})
    assert vaultSavings.paused() == True
...
def test_deposit_array(register_vault, token, vault, vaultSavings, user, deployer):
    ...
    assert (
        vault.available()
        == (vault_totalSupply + DEPOSIT_VALUE) * vault.min() // vault.max()
    )
...
def test_deposit_with_failed_vault(
    register_vault, token, test_vault, vaultSavings, user, deployer
):
    assert vaultSavings.paused() == False
...
def test_deposit_array_with_failed_vault(
    ...
    with brownie.reverts():
        vaultSavings.deposit["address[],uint[]"](
            [test_vault], [DEPOSIT_VALUE], {"from": user}
        )


@given(user=strategy("address", length=10), user2=strategy("address", length=10))
@settings(max_examples=50)
def test_withdraw(register_vault, token, vault, vaultSavings, user, user2, deployer):
    ...
    if user != user2:
        ...
        token.approve(vaultSavings.address, DEPOSIT_VALUE, {"from": user})
        vaultSavings.deposit["address,uint"].transact(
            vault.address, DEPOSIT_VALUE, {"from": user}
        )

        token.approve(vaultSavings.address, DEPOSIT_VALUE, {"from": user2})
        vaultSavings.deposit["address,uint"].transact(
            vault.address, DEPOSIT_VALUE, {"from": user2}
        )
        ...
        vaultSavings.withdraw["address,uint"].transact(
            vault.address, vault.balanceOf(user), {"from": user}
        )

        vault.approve(vaultSavings.address, vault.balanceOf(user2), {"from": user2})
        vaultSavings.withdraw["address,uint"].transact(
            vault.address, vault.balanceOf(user2), {"from": user2}
        )
...
@given(user=strategy("address", length=10), user2=strategy("address", length=10))
@settings(max_examples=50)
def test_withdraw_array(
    register_vault, token, vault, vaultSavings, user, user2, deployer
):
...
@given(user=strategy("address", length=10), vault2=strategy("address", length=10))
@settings(max_examples=50)
def test_deposit_array_valid_faild_vaults(
    register_vault, token, vault, vaultSavings, user, vault2, deployer
):
    ...
    with brownie.reverts():
        vaultSavings.deposit["address[],uint[]"](
            [vault.address, vault2], [DEPOSIT_VALUE, DEPOSIT_VALUE], {"from": user}
        )


@given(user=strategy("address", length=10), vault2=strategy("address", length=10))
@settings(max_examples=50)
def test_withdraw_array_valid_faild_vaults(
    register_vault, token, vault, vaultSavings, user, vault2, deployer
):
