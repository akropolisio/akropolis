...
def prepare_swap(
    deployer, adel, akro, vakro, stakingpool, vakroSwap, exploitCompVAkroSwap
):
...
def test_swap_adel(
    ...
    root, hshs = prepare_root(
        regular_user.address,
        regular_user2.address,
        regular_user3.address,
        regular_user4.address,
    )
    ...
    vakroSwap.swapFromAdel(
        ADEL_TO_SWAP_USER1, 0, AVAILABLE_USER1, [h2, root_2], {"from": regular_user}
    )
    ...
    vakroSwap.swapFromStakedAdel(
        0, AVAILABLE_USER1, [h2, root_2], {"from": regular_user}
    )
...
def test_swap_exploit_comp_vakro(
    chain,
    deployer,
    vakro,
    ...
    with brownie.reverts("Merkle proofs not verified"):
        exploitCompVAkroSwap.swap(
            0, AVAILABLE_USER1, [root_2, h2], {"from": regular_user2}
        )
    for data in user_data:
        ...
        with brownie.reverts("Merkle proofs not verified"):
            exploitCompVAkroSwap.swap(
                0, 1000000e18, [data[3], data[2]], {"from": data[0]}
            )
