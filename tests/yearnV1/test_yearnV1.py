...
def test_withdraw(register_vault, token, vault, vaultSavings, regular_user, deployer):
    ...
    vaultSavings.deposit["address,uint"].transact(
        vault.address, DEPOSIT_VALUE, {"from": deployer}
    )
    ...
    vaultSavings.withdraw["address,uint"].transact(
        vault.address, vault.balanceOf(deployer), {"from": deployer}
    )
...
def test_deposit_unpause_vault_reverts(
    token, vault, vaultSavings, deployer, regular_user
):
    vaultSavings.pause({"from": deployer})
    assert vaultSavings.paused() == True
